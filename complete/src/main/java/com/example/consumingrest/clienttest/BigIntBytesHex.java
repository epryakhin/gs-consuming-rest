package com.example.consumingrest.clienttest;

import org.bouncycastle.util.BigIntegers;
import org.bouncycastle.util.encoders.Hex;

import java.math.BigInteger;
import java.util.Arrays;

public class BigIntBytesHex {

    private static final String HEX = "";

    public static void main(String[] args) {
        BigInteger bigInteger = new BigInteger(1, Hex.decode(HEX));
        byte[] signed = bigInteger.toByteArray();
        System.out.println("As normal: " + Arrays.toString(signed));
        byte[] unsigned = BigIntegers.asUnsignedByteArray(bigInteger);
        System.out.println("As unsigned: " + Arrays.toString(unsigned));
        System.out.println("Equals: " + Arrays.equals(signed, unsigned));
    }
}
